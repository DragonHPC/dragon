...
if __name__ == "__main__":
    ...
    # The following doesn't seem to work for setting the seed when multiple managers and Dragon.
    # We need to set 'export PYTHONHASHSEED=1234' for now.
    ...
    if my_args.dragon:
        ...
        # Convert the total size of the dictionary (in GB) to bytes
    ...
    _keys = generate_keys(dict_size=my_args.dict_size)
    ...
    for i in range(num_batches):
        if (i == num_batches - 1) and (num_keys % batch_size != 0):
            batch_keys = _keys[i * batch_size:]
        else:
            batch_keys = _keys[i * batch_size:(i+1) * batch_size]
        jobs.append(mp.Process(target=assign_keys, args=(dd, batch_keys, value_size)))
        ...
        try:
            ...
            for i in range(num_clients):
                client_proc = mp.Process(target=do_dict_ops,
                                         args=(_keys, dd, i, my_args.iterations,
            ...
            print(f"{value_size}  {result}")
            print(f"Msglen [B]   Rate ")
            print(f"{value_size}  {rate}\n")
