        ...
        try:
            ...
            if dict_ops[ii] == DictOp.DEL_ITEM:
                for i in range(num_clients):
                    grp.add_process(nproc=1, template=
                                    ProcessTemplate(target=do_dict_ops, args=(all_keys[i*my_args.iterations:(i+1)*my_args.iterations], ddict, i, my_args.iterations,
                                                value_size, q, dict_ops[ii])))
            else:
                grp.add_process(nproc=num_clients, template=
                                ProcessTemplate(target=do_dict_ops, args=(all_keys, ddict, 100, my_args.iterations, value_size, q, dict_ops[ii])))
