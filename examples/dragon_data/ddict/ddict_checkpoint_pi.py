...
def proc_function(d: DDict, client_id: int, places: float, trace: bool) -> None:
    ...
    try:
        ...
        while not done:
            ...
            local_avg = ((num_procs * step * num_points) * avg + new_sim * num_points) / ((num_procs * step * num_points) + num_points)
...
def main():
    ...
    parser.add_argument('--digits', type=int, default=10,
                        help='Number of places after the decimal point of PI accuracy')
    parser.add_argument('--trace', nargs='?', type=bool, default=False, const=True,
                        help='Set to True to print simulation result for all clients in each iteration')

    my_args = parser.parse_args()
    places = 5**-(my_args.digits + 1)
    d = DDict(1, 2, int(4 * 1024 * 1024 * 1024), wait_for_keys=True, working_set_size=4, timeout=200)
    procs = []
