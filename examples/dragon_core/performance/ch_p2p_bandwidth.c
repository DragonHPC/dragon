...
dragonChannelDescr_t dragon_channels[NUM_CHANNELS];
...
run_node_0()
{
    ...
    for (int cur_send_size_idx = 0; cur_send_size_idx < MAX_SEND_MSG_SIZES; cur_send_size_idx++) {
        ...
        int window_size = WINDOW_SIZE_BY_MSG_SIZE[cur_send_size_idx];
        ...
        float bandwidth = (msg_size * window_size * samps) / total_time;
        printf("MsgSize = %i, WindowSize = %i, Samps = %i, Bandwidth = %g\n", msg_size, window_size, samps, bandwidth);
