...
class DDictGUPS:
    ...
    def __init__(
        ...
        self._pool = Pool(
            ...
            initargs=(self._ddict, self._barr, self._inq, self._outq, self._stop_ev),
        )

    @classmethod
    def _tester_loop(
        cls, _ddict: DDict, _barr: Barrier, _inq: Queue, _outq: Queue, _stop_ev: Event
    ) -> None:
    ...
    def driver(self):
        ...
        while val_bytes <= self._end_bytes:
            ...
            blks_per_key = (
                ceil(float(def_test.key_bytes) / float(DEFAULT_BLK_SIZE)) + 4
            )  # extra is for pickle header
            blks_per_val = (
                ceil(float(val_bytes) / float(DEFAULT_BLK_SIZE)) + 4
            )  # extra is for pickle header
            op_blks = self._nclients * (blks_per_key + blks_per_val)
            avail_blks = floor(
                float(self._mem_frac * self._tot_dict_bytes) / DEFAULT_BLK_SIZE
            )
            ...
            the_test = DDictTest(
                op=DDictTestOp.PUT, iterations=iters, value_bytes=val_bytes, nkeys=nkeys
            )
            self._ddict.clear()
            ...
            if val_bytes <= DEFAULT_BLK_SIZE:
                iters = max(
                    1, int(float(iters) * 0.5)
                )  # to account for number of keys not changing until we hit the block size
            else:
                iters += int(
                    isize * 1.2
                )  # more iterations because fewer ops needed to fill ddict
                isize += 1
            val_bytes *= 2
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stats", type=bool, default=False, help="print ddict stats for each iteration"
    )
    ...
    if my_args.num_nodes > my_system.nnodes:
        raise ValueError(
            "More nodes requested for the DDict than available in the runtime"
        )

    print("DDict GUPS Benchmark", flush=True)
    print(
        f"  Running on {my_system.nnodes} nodes (nclients={my_args.nclients})",
        flush=True,
    )
    print(
    ...
    print(
        f"  {my_args.total_mem_size} GB total DDict memory ({my_args.mem_frac * my_args.total_mem_size} GB for keys+values)",
        flush=True,
    )
