...
Allocations from managed memory pools is implemented using a heap manager that uses the buddy allocator algorithm.
...
We provide a Cython adapter to :ref:`ManagedMemory`:
...
.. .. c:function:: dragonError_t dragon_memory_pool_allocation_exists(dragonMemoryDescr_t * mem_descr, int * flag)

..     Searches the manifest for an allocation matching the given memory descriptor.
