...
def local_executor(
    user_command: List[str], env: dict, cwd: str, ref: int, my_rank: int, num_ranks: int, send_msg_q: Queue
):
    logger.debug("++local_executor(%d, %s, %s, %s, %d)", ref, user_command, str(env), cwd, my_rank)
    ...
    try:
        ...
        send_msg_q.put(
            messages.UserAppExit(util.next_tag(), ref=ref, hostname=hostname, exit_code=user_process.returncode)
        )
