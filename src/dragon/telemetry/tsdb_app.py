...
class TSDBApp(Flask):
    """Custom Flask Application for TSDB Server"""
    ...
    def get_metrics_from_db(self) -> list:
        ...
        res = list(self.cursor.execute(sql))
        res = [x[0] for x in res]
        return res

    def set_filename(self, filename: str) -> None:
        """Sets the filename for the database connection
    ...
    def get_filename(self) -> str:
        """Gets the filename for the database connection
...
def add_metrics() -> object:
    ...
    metrics_list = app.get_metrics_from_db()
    success = 0
    failed = 0
    error_msg = []
    ...
    for req in req_body["dps"]:
        ...
        try:
            ...
            app.cursor.execute(sql_insert, [metric, str(timestamp), req["value"], json.dumps(tags)])
...
def set_is_shutdown() -> object:
    ...
    try:
        ...
        shutdown_event = loads(list(app.cursor.execute(sql).fetchone())[0])
        shutdown_event.set()
        success += 1
...
def cleanup_tsdb() -> object:
    ...
    res = list(app.cursor.execute(select_sql, [int(start_time)]))
    app.connection.commit()
    success = 0
    failed = 0
    error_msg = []
