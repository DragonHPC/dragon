...
def dragon_wait_for_engine_startup(
    ...
    while any(conn_pending) or any(start_pending):
        ...
        events = handshake_socket.poll(timeout=STARTUP_POLL_PERIOD_MS, flags=zmq.POLLIN)
        ...
        if events == 0:
            ...
            if ready:
                ...
                raise RuntimeError(
                    "Engine core initialization failed. " "See root cause above. " f"Failed core proc(s): {finished}"
                )
        ...
        if engine is None:
            ...
            raise RuntimeError(f"Message from engine with unexpected data " f"parallel rank: {eng_index}")
        ...
        if not local and headless != remote_should_be_headless:
            if headless:
                raise RuntimeError(
                    f"Remote engine {eng_index} must not use " f"--headless in external or hybrid dp lb " f"mode"
                )
            else:
                raise RuntimeError(
                    f"Remote engine {eng_index} must use " f"--headless unless in external or hybrid " f"dp lb mode"
                )
