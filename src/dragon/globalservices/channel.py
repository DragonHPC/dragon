...
def create(m_uid, user_name='', options=None, soft=False, puid_override=None):
    ...
    req_msg = dmsg.GSChannelCreate(tag=das.next_tag(), p_uid=puid,
                                   r_c_uid=das.get_gs_ret_cuid(), m_uid=m_uid,
                                   user_name=user_name, options=options)
...
def get_list():
    """Asks Global Services for a list of the c_uids of all currently active channels
        ...
        """
    req_msg = dmsg.GSChannelList(tag=das.next_tag(), p_uid=this_process.my_puid,
...
def query(identifier, *, inc_refcnt=False):
    ...
    if isinstance(identifier, str):
        req_msg = dmsg.GSChannelQuery(tag=das.next_tag(), p_uid=this_process.my_puid,
                                      r_c_uid=das.get_gs_ret_cuid(),
                                      inc_refcnt=inc_refcnt, user_name=identifier)
    else:
        req_msg = dmsg.GSChannelQuery(tag=das.next_tag(), p_uid=this_process.my_puid,
                                      r_c_uid=das.get_gs_ret_cuid(), inc_refcnt=inc_refcnt,
...
def join(identifier, timeout=None):
    ...
    req = dmsg.GSChannelJoin(tag=das.next_tag(), p_uid=this_process.my_puid,
                             r_c_uid=das.get_gs_ret_cuid(), name=identifier,
                             timeout=timeout)
...
def destroy(identifier, *, just_decref=False):
    ...
    if isinstance(identifier, str):
        req_msg = dmsg.GSChannelDestroy(tag=das.next_tag(), p_uid=this_process.my_puid,
                                        r_c_uid=das.get_gs_ret_cuid(), dec_ref=just_decref,
                                        reply_req=rreq,  user_name=identifier)
    else:
        req_msg = dmsg.GSChannelDestroy(tag=das.next_tag(), p_uid=this_process.my_puid,
                                        r_c_uid=das.get_gs_ret_cuid(),dec_ref=just_decref,
                                        reply_req=rreq, c_uid=int(identifier))
