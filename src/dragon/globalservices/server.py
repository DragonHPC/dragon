...
from ..infrastructure.policy import GS_DEFAULT_POLICY
...
class GlobalContext(object):
    ...
    def __init__(self, test_gs_stdout=None):
        ...
        self.resource_to_group_map = dict()  # key = resource uid e.g. p_uid, value = tuple including g_uid, and position of the resource in the GSGroupCreate.items list
        self.group_to_pending_resource_map = dict() # used for pending resources of a group, key = (msg tag, g_uid), value = resource_context
        self.group_resource_count = dict()  # key = g_uid, value = list of items corresponding to the multiplicity of each list item in GroupDescriptor.sets
        self.group_destroy_resource_count = dict()  # is applied only when destroy has been called, key = g_uid, value = list of items corresponding to the multiplicity of each list item in GroupDescriptor.sets
        ...
        self.pending_join_list = dutil.PriorityMultiMap()  # pending joins, key = frozenset of p_uid, val = set of join req for this set of p_uid
        self.pending_channel_joins = dutil.PriorityMultiMap()  # pending channel joins  key = channel names

        self.puid_join_list_status = dict()  # this is a dict of dicts that holds the p_uid status for every muti-join wait request
    ...
    def get_reply_handle(self, msg):
        ...
        if self._launch_mode in {self.LaunchModes.MULTI,
            ...
            if 1 == msg.r_c_uid:
            ...
            elif dfacts.BASE_BE_CUID <= msg.r_c_uid < (
                    dfacts.BASE_BE_CUID + dfacts.RANGE_BE_CUID):
                log.debug("this is bela")
                return self.bela_input
            elif dfacts.BASE_SHEP_CUID <= msg.r_c_uid < (
                    dfacts.BASE_SHEP_CUID + dfacts.RANGE_SHEP_CUID):
                ...
                if context.attached_connection is None:
                    ...
                    if context.is_reading:
                        context.attached_connection = dconn.Connection(inbound_initializer=underlying_channel, policy=dparm.POLICY_INFRASTRUCTURE)
                    else:
                        context.attached_connection = dconn.Connection(outbound_initializer=underlying_channel, policy=dparm.POLICY_INFRASTRUCTURE)
    ...
    def resolve_puid(self, name=None, p_uid=None):
        """Checks if a process exists given name and/or p_uid.
                ...
                """
        return self._resolve_uid(name, p_uid,
                                 self.process_names, self.process_table,
                                 'process', 'p_uid')
    ...
    def resolve_muid(self, name, m_uid=-1):
        ...
        """
        return self._resolve_uid(name, m_uid,
                                 self.pool_names, self.pool_table,
                                 'pool', 'm_uid')
    ...
    def resolve_cuid(self, name, c_uid=-1):
        """Checks if a channel exists given name and/or c_uid.
            ...
            see docstring on _resolve_puid for more details.
        """
        return self._resolve_uid(name, c_uid,
                                 self.channel_names, self.channel_table,
                                 'channel', 'c_uid')
    ...
    def resolve_huid(self, name, h_uid=-1):
        """Checks if a node exists given name and/or h_uid.
            ...
            see docstring on _resolve_puid for more details.
        """
        return self._resolve_uid(name, h_uid,
                                 self.node_names, self.node_table,
                                 'node', 'h_uid')
    ...
    def resolve_guid(self, name, g_uid=-1):
        """Checks if a group exists given name and/or g_uid.
            ...
            see docstring on _resolve_puid for more details.
        """
        return self._resolve_uid(name, g_uid,
                                 self.group_names, self.group_table,
                                 'group', 'g_uid')
    ...
    def run_global_server(self, mode=LaunchModes.SINGLE, test_gs_input=None,
                          test_shep_inputs=None,
                          test_bela_input=None, test_gs_stdout=None, test_conns=None):
    ...
    def run_startup(self, mode=LaunchModes.SINGLE, test_gs_input=None, test_shep_inputs=None,
                    test_bela_input=None, test_gs_stdout=None):
        ...
        if self.LaunchModes.SINGLE == self._launch_mode:
            ...
            my_inputs = startup.startup_single(self, test_gs_input,
                                               test_shep_inputs[0],
                                               test_bela_input)
            ...
            my_inputs = startup.startup_multi(self,
                                              test_gs_input,
                                              test_shep_inputs,
                                              test_bela_input)
            self.shep_inputs, self.gs_input, self.bela_input, self.num_nodes = my_inputs
    ...
    def run_teardown(self, test_gs_input=None, test_shep_inputs=None,
                     test_bela_input=None, test_gs_stdout=None):
    ...
    def handle_sh_ping(self, msg):
        ...
        if self.sh_pings_recvd == self.num_nodes:
            ...
            self.policy_eval = PolicyEvaluator(node_descrs, GS_DEFAULT_POLICY)
    ...
    def handle_process_list(self, msg):
        ...
        rm = dmsg.GSProcessListResponse(tag=self.tag_inc(),
                                        ref=msg.tag,
    ...
    def handle_process_query(self, msg):
        ...
        if not found:
            rm = dmsg.GSProcessQueryResponse(tag=self.tag_inc(),
                                             ref=msg.tag,
                                             err=dmsg.GSProcessQueryResponse.Errors.UNKNOWN,
                                             err_info=errmsg)
        else:
            pdesc = self.process_table[target_uid].descriptor
            rm = dmsg.GSProcessQueryResponse(tag=self.tag_inc(),
                                             ref=msg.tag,
                                             err=dmsg.GSProcessQueryResponse.Errors.SUCCESS,
                                             desc=pdesc)
    ...
    def handle_process_join(self, msg):
        ...
        if not found:
            rm = gspjr(tag=self.tag_inc(),
                       ref=msg.tag,
                       err=gspjr.Errors.UNKNOWN,
                       err_info=errmsg)
            reply_channel.send(rm.serialize())
            log.debug(f'unknown: response to {msg}: {rm}')
        elif target_uid == msg.p_uid:
            rm = gspjr(tag=self.tag_inc(), ref=msg.tag,
                       err=gspjr.Errors.SELF, err_info='self-join forbidden')
            reply_channel.send(rm.serialize())
    ...
    def handle_process_exit(self, msg):
        ...
        if ctx.request.options.make_inf_channels:
            clean_chan = dmsg.GSChannelDestroy(tag=self.tag_inc(),
                                               p_uid=dfacts.GS_PUID, r_c_uid=0,
                                               c_uid=ctx.descriptor.gs_ret_cuid)
            ChannelContext.destroy(self, clean_chan, dutil.AbsorbingChannel())

        if ctx.stdin_context is not None:
            clean_chan = dmsg.GSChannelDestroy(tag=self.tag_inc(),
                                               p_uid=dfacts.GS_PUID, r_c_uid=0,
                                               c_uid=ctx.stdin_context.shchannelcreate_msg.c_uid, dec_ref=True)
            ChannelContext.destroy(self, clean_chan, dutil.AbsorbingChannel())

        if ctx.stdout_context is not None:
            clean_chan = dmsg.GSChannelDestroy(tag=self.tag_inc(),
                                               p_uid=dfacts.GS_PUID, r_c_uid=0,
                                               c_uid=ctx.stdout_context.shchannelcreate_msg.c_uid, dec_ref=True)
            ChannelContext.destroy(self, clean_chan, dutil.AbsorbingChannel())

        if ctx.stderr_context is not None:
            clean_chan = dmsg.GSChannelDestroy(tag=self.tag_inc(),
                                               p_uid=dfacts.GS_PUID, r_c_uid=0,
                                               c_uid=ctx.stderr_context.shchannelcreate_msg.c_uid, dec_ref=True)
            ChannelContext.destroy(self, clean_chan, dutil.AbsorbingChannel())
        ...
        for oldset, puidset, joinreq in to_be_added:
            if joinreq.timeout < 0:
                ...
                timeout = self.pending_join_list.time_left(oldset, joinreq) - time.time()  # this is the remaining time on waiting until timeout
    ...
    def handle_pool_list(self, msg):
        ...
        rm = dmsg.GSPoolListResponse(tag=self.tag_inc(),
                                     ref=msg.tag,
    ...
    def handle_pool_query(self, msg):
        ...
        if not found:
            rm = dmsg.GSPoolQueryResponse(tag=self.tag_inc(),
                                          ref=msg.tag,
                                          err=dmsg.GSPoolQueryResponse.Errors.UNKNOWN,
                                          err_info=errmsg)
        else:
            pdesc = self.pool_table[target_uid].descriptor
            rm = dmsg.GSPoolQueryResponse(tag=self.tag_inc(),
                                          ref=msg.tag,
                                          err=dmsg.GSPoolQueryResponse.Errors.SUCCESS,
                                          desc=pdesc)
    ...
    def handle_channel_list(self, msg):
        ...
        rm = dmsg.GSChannelListResponse(tag=self.tag_inc(),
                                        ref=msg.tag,
    ...
    def handle_channel_query(self, msg):
        ...
        if not found:
            rm = dmsg.GSChannelQueryResponse(tag=self.tag_inc(),
                                             ref=msg.tag,
                                             err=dmsg.GSChannelQueryResponse.Errors.UNKNOWN,
                                             err_info=errmsg)
        else:
    ...
    def handle_channel_join(self, msg):
        ...
        if found:
            ...
            if cdesc.State.ACTIVE == cdesc.state:
                reply_channel = self.get_reply_handle(msg)
                rm = dmsg.GSChannelJoinResponse(tag=self.tag_inc(), ref=msg.tag,
                ...
                reply_channel = self.get_reply_handle(msg)
                rm = dmsg.GSChannelJoinResponse(tag=self.tag_inc(),
                                                ref=msg.tag,
                                                err=dmsg.GSChannelJoinResponse.Errors.DEAD,
                                                desc=cdesc)
                reply_channel.send(rm.serialize())
                log.debug(f'response to {msg!s}: {rm!s}')
                handled_immediately = True
    ...
    def handle_node_list(self, msg):
        ...
        rm = dmsg.GSNodeListResponse(tag=self.tag_inc(),
                                     ref=msg.tag,
    ...
    def handle_node_query_total_cpu_count(self, msg):
        ...
        rm = dmsg.GSNodeQueryTotalCPUCountResponse(tag=self.tag_inc(), ref=msg.tag,
    ...
    def handle_node_query(self, msg):
        ...
        if not found:
            log.error(errmsg)
            rm = dmsg.GSNodeQueryResponse(tag=self.tag_inc(),
                                          ref=msg.tag,
                                          err=dmsg.GSNodeQueryResponse.Errors.UNKNOWN,
                                          err_info=errmsg)
        else:
            rm = dmsg.GSNodeQueryResponse(tag=self.tag_inc(), ref=msg.tag,
                                          err=dmsg.GSNodeQueryResponse.Errors.SUCCESS,
                                          desc=sdesc)
...
def single():
    dlog.setup_logging(basename=os.environ.get(dfacts.GS_LOG_BASE, 'gs'),
                       level=logging.DEBUG)
