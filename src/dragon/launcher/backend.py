...
class LauncherBackEnd:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if self._abnormally_terminating:
            log.debug('raising RuntimeError')
    ...
    def run_startup(self,
        ...
        try:
            ...
            self.be_mpool = MemoryPool(int(dfacts.DEFAULT_BE_OVERLAY_TRANSPORT_SEG_SZ),
            ...
            self.local_inout = Connection(inbound_initializer=self.local_ch_in,
                                          ...
                                          options=conn_options,
                                          policy=conn_policy)
            self.infra_in = Connection(inbound_initializer=self.be_inbound,
                                       options=conn_options,
                                       policy=conn_policy)

            # Create a backdoor into sending messages to infra_in in case I need
            # to tell the thread using it to shutdown:
            self.infra_in_bd = Connection(outbound_initializer=self.be_inbound,
                                          options=conn_options,
                                          policy=conn_policy)
            ...
            conn_options = ConnectionOptions(default_pool=self.be_mpool, min_block_size=2 ** 16)
            self.infra_out = Connection(outbound_initializer=be_outbound,
                                        options=conn_options,
                                        policy=conn_policy)
            self.infra_out.ghost = True

        except (ChannelError, DragonPoolError,
        ...
        try:
            self.tree_proc = start_overlay_network(ch_in_sdesc=B64(self.local_ch_out.serialize()),
        ...
        be_up_msg = dmsg.BEIsUp(tag=dlutil.next_tag(), be_ch_desc=be_ch_desc,
        ...
        self.recv_msgs_from_overlaynet_thread = threading.Thread(name='Frontend Monitor',
                                                                 target=self.receive_messages_from_overlaynet,
                                                                 args=recv_msgs_from_overlaynet_thread_args, daemon=False)
        ...
        self.send_msgs_to_overlaynet_thread = threading.Thread(name='Backend Monitor',
                                                               target=self.send_messages_to_overlaynet,
                                                               args=send_msgs_to_overlaynet_thread_args, daemon=False)
        ...
        self.send_log_to_overlaynet_thread = threading.Thread(name='Logging Monitor',
                                                              target=self.send_log_msgs_to_overlaynet,
                                                              args=send_log_to_overlaynet_thread_args, daemon=False)
        ...
        self.ls_stdin = NewlineStreamWrapper(os.fdopen(stdin_fd, 'wb', buffering=0), read_intent=False, write_intent=True)
        self.ls_stdout = NewlineStreamWrapper(os.fdopen(stdout_fd, 'rb', buffering=0), read_intent=True, write_intent=False)
        ...
        self.channel_monitor_thread = threading.Thread(name='BE Channel Monitor',
                                                       target=self.be_channel_monitor,
                                                       args=channel_monitor_args, daemon=False)
            ...
            try:
                try:
                    ...
                    if self._state < BackendState.TEARDOWN:
                        ...
                        log.debug('Caught an abnormal termination during teardown. Setting to flag to raise Exception at exit.')
                        self._abnormally_terminating = True
                        continue
    ...
    def handle_gs_teardown(self, msg: dmsg.GSTeardown):
        ...
        try:
            ...
            self.msg_log.info('Received a GSTeardown before backend fully up. Will work towards cleaning backend and exit')
    ...
    def handle_sh_halt_ta(self, msg: dmsg.SHHaltTA):
        ...
        try:
            ...
            self.msg_log.info('Received a SHHaltTA before backend fully up. Will work towards cleaning backend and exit')
            self._abnormal_termination(msg)
