...
class Orchestrator:
    ...
    def __init__(self, managers_per_node, n_nodes, total_mem):
        try:
            ...
            self._num_managers = (n_nodes * managers_per_node)
    ...
    def run(self):
        ...
        self._free_resources(msg)
        log.info('Exiting orchestrator.')

    def _free_resources(self, msg):
        self._manager_pool.join()
        self._manager_pool.stop()
        ...
        resp_msg = dmsg.DDDestroyResponse(self._tag_inc(), ref=msg.tag, err=DragonError.SUCCESS)
    ...
    def register_client(self, msg: dmsg.DDRegisterClient) -> None:
        resp_msg = dmsg.DDRegisterClientResponse(tag=self._tag_inc(), ref=msg.tag, err=DragonError.SUCCESS, clientID=self._clientid_inc(), numManagers=self._num_managers)
    ...
    def create(self, msg: dmsg.DDCreate) -> None:
        ...
        template = ProcessTemplate(manager_proc, args=(self.mpool_size, self._serialized_return_connector, self._serialized_main_connector, args), policy=self._policy)
        ...
        try:
            ...
            if self._num_managers_created == self._num_managers:
                ...
                for i, (m, tag) in enumerate(self._serialized_manager_return_flis):
                    resp_msg = dmsg.DDRegisterManagerResponse(self._tag_inc(), ref=tag, err=DragonError.SUCCESS, managerID=i, managers=self._serialized_manager_flis)
...
def start(managers_per_node: int, n_nodes: int, total_mem: int):
    try:
        log.debug('Initing Orchestrator')
        orc = Orchestrator(managers_per_node, n_nodes, total_mem)
        orc.run()
    except Exception as ex:
        tb = traceback.format_exc()
