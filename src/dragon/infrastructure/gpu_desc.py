...
def find_nvidia() -> list:
    ...
    return merge_masked_detected(mask, devices)
...
def find_accelerators() -> AcceleratorDescriptor:
    ...
    for env_str in [AccEnvStr.NVIDIA, AccEnvStr.AMD, AccEnvStr.INTEL]:
        mask = os.environ.get(env_str, None)
        if mask is not None:
            devices.device_list = merge_masked_detected(mask, devices.device_list)
