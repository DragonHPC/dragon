...
class TestLock(unittest.TestCase):
    def test_lock_basic(self):
        ...
        # all should be waiting, we still own lock
        live_puids = get_list()
        ...
        # release one
        lock.release()
        ready = multi_join(puids, timeout=None)
        self.assertTrue(len(ready[0]) == 1)  # first has exited
        puids.remove(ready[0][0][0])  # wow ...

        # release two
        lock.release()
        ready = multi_join(puids, timeout=None, join_all=True)
        self.assertTrue(len(ready[0]) == 1)  # rest has exited

        # cause a value error
        lock.release()  # one release left ...
