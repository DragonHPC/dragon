...
class TestAutomation:
    def __init__(self, open_file_desc_limit):
        ...
        self.directory_name = (
            "results" + str((subprocess.check_output("hostname").decode()).strip()) + str(datetime.datetime.now())
        )
    ...
    def __get_args(self):
        ...
        parser.add_argument(
            "--basic_process_test", action="store_true", help="run suite of benchmarks for basic_process_test.py"
        )
        parser.add_argument(
            "--basic_pool_test", action="store_true", help="run suite of benchmarks for basic_pool_test.py"
        )
        parser.add_argument(
            "--pool_invocation_test", action="store_true", help="run suite of benchmarks for pool_invocation_test.py"
        )
        parser.add_argument(
            "--send_receive_test", action="store_true", help="run suite of benchmarks for send_receive_test.py"
        )
        parser.add_argument(
            "--chain_token_test", action="store_true", help="run suite of benchmarks for chain_token_test.py"
        )
        parser.add_argument(
            "--chain_token_pipe_test", action="store_true", help="run suite of benchmarks for chain_token_pipe_test.py"
        )
        parser.add_argument(
            "--chain_token_test_simple",
            action="store_true",
            help="run suite of benchmarks for chain_token_test_simple.py",
        )
        parser.add_argument(
            "--basic_managed_dict_test",
            action="store_true",
            help="run suite of benchmarks for basic_managed_dict_test.py",
        )
        parser.add_argument(
            "--access_managed_dict_test",
            action="store_true",
            help="run suite of benchmarks for access_managed_dict_test.py",
        )
        parser.add_argument(
            "--num_iterations", type=int, default=None, help="number of iteration for each instance of run tests"
        )
        parser.add_argument(
            "--spins",
            type=int,
            default=None,
            help="number of spins for each instance of a test with spins." + " can lead to duplicated tests.",
        )
        parser.add_argument(
            "--max_workers", type=int, default=self.cpus, help="maximum number of workers a test will run with"
        )
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/basic_pool_test.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
                            "--json",
                        ]
                    ).decode()
                )
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/send_receive_test.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
                            "--message_size",
                            input_dict["message_size"][i],
                            "--response_size",
                            input_dict["response_size"][i],
                            "--json",
                        ]
                    ).decode()
                )
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/chain_token_pipe_test.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/chain_token_test_simple.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/basic_managed_dict_test.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
            ...
            if int(input_dict["num_workers"][i]) <= self.args.max_workers:
                output.append(
                    subprocess.check_output(
                        [
                            sys.executable,
                            "tests/access_managed_dict_test.py",
                            "--num_workers",
                            input_dict["num_workers"][i],
                            "--num_iterations",
                            input_dict["num_iterations"][i],
        ...
        output.append(
            subprocess.check_output(
                [
                    ...
                    "tests/access_managed_dict_test.py",
                    "--num_workers",
                    str(int(self.args.max_workers / 2)),
                    "--num_iterations",
                    "5",
                    "--json",
                ]
