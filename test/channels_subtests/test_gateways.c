...
fake_transport(const dragonChannelSerial_t* gw_ser)
{
    ...
    while (count < 8) {
        ...
        if (gw_msg.msg_kind == DRAGON_GATEWAY_MESSAGE_EVENT) {
            ...
            if (gw_msg.event_mask == DRAGON_CHANNEL_CLEANUP) {
                /* These events are ignored. They are cleanup events for a transport and
                   this fake one doesn't care. These events are generated by calling
                   dragon_channel_destroy. They are not generated by users. */
                count -= 1;
            } else {
