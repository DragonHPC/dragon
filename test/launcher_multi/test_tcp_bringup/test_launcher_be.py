...
def get_args():
    ...
    parser = argparse.ArgumentParser(prog='launcher-be',
                                     description='Arguments for standing up backend infrastructure tree for Dragon')
...
if __name__ == '__main__':
    ...
    be_mpool = MemoryPool(int(dfacts.DEFAULT_SINGLE_DEF_SEG_SZ),
    ...
    tree_proc = start_channel_tree(args.node_index,
                                   str(B64(local_ch.serialize())),
                                   encoded_ser_gw_str,
                                   args.host_ids,
                                   args.ip_addrs)
