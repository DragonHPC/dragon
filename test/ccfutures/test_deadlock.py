...
class ExecutorDeadlockTest:
    ...
    def test_gh105829_should_not_deadlock_if_wakeup_pipe_full(self):
        ...
        with unittest.mock.patch.object(futures.process._ExecutorManagerThread, "run", mock_run), unittest.mock.patch(
            "concurrent.futures.process._ThreadWakeup", MockWakeup
        ):
            with self.executor_type(max_workers=2, mp_context=self.get_context()) as executor:
                self.executor = executor  # Allow clean up in fail_on_deadlock
