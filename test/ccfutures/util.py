...
class InterpreterPoolExecutor(concurrent.futures.ProcessPoolExecutor):
    def __init__(self, max_workers=None):
        super().__init__(max_workers=max_workers or multiprocessing.cpu_count())
    ...
    def setUp(self):
        ...
        if hasattr(self, "ctx"):
            self.executor = self.executor_type(
                max_workers=self.worker_count, mp_context=self.get_context(), **self.executor_kwargs
            )
        else:
...
class InterpreterPoolMixin(ExecutorMixin):
    executor_type = InterpreterPoolExecutor
