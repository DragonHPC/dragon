...
from concurrent.futures._base import PENDING, RUNNING, CANCELLED, CANCELLED_AND_NOTIFIED, FINISHED, Future
...
class FutureTests(BaseTestCase):
    ...
    def test_multiple_set_result(self):
        ...
        with self.assertRaisesRegex(
            futures.InvalidStateError, "FINISHED: <Future at 0x[0-9a-f]+ " "state=finished returned int>"
        ):
            f.set_result(2)
    ...
    def test_multiple_set_exception(self):
        ...
        with self.assertRaisesRegex(
            futures.InvalidStateError, "FINISHED: <Future at 0x[0-9a-f]+ " "state=finished raised ValueError>"
        ):
