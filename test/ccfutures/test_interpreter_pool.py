...
class BrokenInterpeterPool(Exception):
    pass
...
from multiprocessing import Queue as queues
...
def get_current_interpid(*extra):
    ...
    interpid = id(sys.modules["__main__"])
...
@unittest.skip("Dragon does not support sending file descriptors to other processes.")
class InterpreterPoolExecutorTest(InterpretersMixin, ExecutorTest, BaseTestCase):
    ...
    def test_init_func(self):
        ...
        executor = self.executor_type(initializer=write_msg, initargs=(w, msg))
    ...
    def test_init_exception_in_script(self):
        ...
        self.assertIn("The above exception was the direct cause of the following exception:", stderr)

    @unittest.skip("Dragon does not support sending file descriptors to other processes.")
    def test_init_exception_in_func(self):
        executor = self.executor_type(initializer=fail, initargs=(Exception, "spam"))
        with executor:
            with contextlib.redirect_stderr(io.StringIO()) as stderr:
    ...
    def test_submit_exception_in_script(self):
        ...
        for attr in ("__name__", "__qualname__", "__module__"):
            self.assertEqual(getattr(cause.excinfo.type, attr), getattr(Exception, attr))
...
@unittest.skip("AsyncioTest does not work.")
class AsyncioTest(InterpretersMixin, IsolatedAsyncioTestCase):
    ...
    def test_run_in_executor(self):
        ...
        unexpected = id(sys.modules["__main__"])
