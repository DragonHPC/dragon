...
_syscall_regex = re.compile(r"(?P<syscall>[^(]*)\((?P<args>[^)]*)\)\s*[=]\s*(?P<returncode>.+)")
_returncode_regex = re.compile(rb"\+\+\+ exited with (?P<returncode>\d+) \+\+\+")
...
class StraceResult:
    ...
    def events(self):
        ...
        matches = [_syscall_regex.match(event) for event in decoded_events.splitlines()]
        return [
            StraceEvent(match["syscall"], [arg.strip() for arg in (match["args"].split(","))], match["returncode"])
    ...
    def sections(self):
        ...
        for event in self.events():
            if event.syscall == "write" and len(event.args) > 2 and event.args[1].startswith('"MARK '):
                # Found a new section, don't include the write in the section
                # but all events until next mark should be in that section
                current_section = event.args[1].split(" ", 1)[1].removesuffix('\\n"')
...
def strace_python(code, strace_flags, check=True):
    ...
    if len(output) != 2:
        return _make_error("Expected strace events and exit code line", stripped[-50:])

    returncode_match = _returncode_regex.match(output[1])
    if not returncode_match:
        return _make_error("Expected to find returncode in last line.", output[1][:50])
