        ...
        if hasattr(socket, "SO_REUSEPORT"):
            try:
                if sock.getsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT) == 1:
                    raise support.TestFailed(
                        "tests should never set the " "SO_REUSEPORT socket option on " "TCP/IP sockets!"
                    )
            except OSError:
...
def create_unix_domain_name():
    ...
    """
    return tempfile.mktemp(prefix="test_python_", suffix=".sock", dir=os.path.curdir)
...
def _get_sysctl(name):
    ...
    if proc.returncode:
        support.print_warning(f"{' '.join(cmd)!r} command failed with " f"exit code {proc.returncode}")
    ...
    try:
        ...
        support.print_warning(f"Failed to parse {' '.join(cmd)!r} " f"command output {output!r}: {exc!r}")
        # cache the error to only log the warning once
        _sysctl_cache[name] = None
        return None
...
def skip_if_tcp_blackhole(test):
    ...
    skip_if = unittest.skipIf(tcp_blackhole(), "TCP blackhole is enabled (sysctl net.inet.tcp.blackhole)")
    return skip_if(test)
