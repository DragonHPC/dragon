...
def _requires_unix_version(sysname, min_version):
    ...
    return unittest.skipIf(skip, f"{sysname} version {min_version_txt} or higher required, not " f"{version_txt}")
...
def check_sanitizer(*, address=False, memory=False, ub=False, thread=False):
    ...
    memory_sanitizer = "-fsanitize=memory" in cflags or "--with-memory-sanitizer" in config_args
    address_sanitizer = "-fsanitize=address" in cflags or "--with-address-sanitizer" in config_args
    ub_sanitizer = "-fsanitize=undefined" in cflags or "--with-undefined-behavior-sanitizer" in config_args
    thread_sanitizer = "-fsanitize=thread" in cflags or "--with-thread-sanitizer" in config_args
    return (
...
def skip_android_selinux(name):
    return unittest.skipIf(sys.platform == "android", f"Android blocks {name} with SELinux")
...
has_socket_support = not (is_emscripten or is_wasi)
...
def requires_specialization(test):
    return unittest.skipUnless(_opcode.ENABLE_SPECIALIZATION, "requires specialization")(test)


def requires_specialization_ft(test):
    return unittest.skipUnless(_opcode.ENABLE_SPECIALIZATION_FT, "requires specialization")(test)
...
def skip_if_buggy_ucrt_strfptime(test):
    ...
    if _buggy_ucrt is None:
        if sys.platform == "win32" and locale.getencoding() == "cp65001" and time.localtime().tm_zone == "":
            _buggy_ucrt = True
        else:
            _buggy_ucrt = False
    ...
    def __enter__(self):
        ...
        if sys.platform.startswith("win"):
            ...
            if sys.platform == "darwin":
                ...
                cmd = ["/usr/bin/defaults", "read", "com.apple.CrashReporter", "DialogType"]
                proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                with proc:
                    stdout = proc.communicate()[0]
                if stdout.strip() == b"developer":
                    print("this test triggers the Crash Reporter, " "that is intentional", end="", flush=True)
...
def setup_venv_with_pip_setuptools_wheel(venv_dir):
    ...
    with temp_cwd() as temp_dir:
        ...
        cmd = [python, "-X", "dev", "-m", "pip", "install", _findwheel("setuptools"), _findwheel("wheel")]
        run_command(cmd)
...
def sleeping_retry(timeout, err_msg=None, /, *, init_delay=0.010, max_delay=1.0, error=True):
    """
