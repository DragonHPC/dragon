    ...
    try:
        ...
        try:
            ...
            if not quiet:
                raise
            warnings.warn(
                f"tests may fail, unable to restore the mode of " f"{path!r} to {saved_mode.st_mode}: {exc}",
                RuntimeWarning,
                stacklevel=3,
            )
...
try:
    ...
    def subst_drive(path):
        ...
        for c in reversed(string.ascii_uppercase):
            drive = f"{c}:"
            if not kernel32.QueryDosDeviceW(drive, None, 0) and ctypes.get_last_error() == ERROR_FILE_NOT_FOUND:
                break
        else:
        ...
        try:
            ...
            if not kernel32.DefineDosDeviceW(DDD_REMOVE_DEFINITION | DDD_EXACT_MATCH_ON_REMOVE, drive, path):
                raise ctypes.WinError(ctypes.get_last_error())
